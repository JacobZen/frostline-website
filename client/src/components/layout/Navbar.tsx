...
export default function Navbar() {
  ...
  return (
    <nav 
      className="sticky top-0 z-50 h-16 bg-white dark:bg-[#081620] border-b border-gray-200 dark:border-gray-800 shadow-[0_4px_20px_rgba(12,30,40,0.08)] dark:shadow-[0_4px_20px_rgba(0,0,0,0.25)]"
      role="navigation"
    ...
    >
                ...
                <Link key={link.path} href={link.path} data-testid={`link-${link.label.toLowerCase().replace(' ', '-')}`}>
                  <div
                    className={`flex-shrink-0 px-3 py-1.5 rounded-md text-sm font-medium transition-all duration-150 ease-out cursor-pointer ${
                      isActive(link.path)
                        ? 'bg-[#0c1e28]/10 text-[#0c1e28] dark:bg-[#00e892]/15 dark:text-[#00e892]'
                        : 'text-gray-900 dark:text-gray-100 hover:text-[#0c1e28] dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 hover:scale-[1.03]'
                    }`}
      ...
      >
        ...
        <div 
          className={`absolute inset-0 bg-white dark:bg-[#081620] transition-opacity duration-150 ease-out ${
